...
CREATE QUEUE server_a_service_queue
    WITH 
         STATUS = ON
       , RETENTION = OFF
       , ACTIVATION (
...
CREATE SERVICE server_a_service
    ON QUEUE server_a_service_queue (
        service_contract);

CREATE ROUTE server_b_service_route
	WITH 
        SERVICE_NAME = 'server_b_service',
	    ADDRESS	= 'TCP://server_b:4022';
GO

-- Allow all users to initiate conversations
GRANT SEND ON SERVICE::[server_a_service] TO PUBLIC;
GO

-- Create the service broker endpoint 
-- and sign it using the certificate created in broker_certificate_creation.sql
CREATE ENDPOINT server_a_service_endpoint
    STATE = STARTED
    AS TCP (LISTENER_PORT = 4022)
    FOR SERVICE_BROKER (
        AUTHENTICATION = CERTIFICATE server_a_certificate);
GO